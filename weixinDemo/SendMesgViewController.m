...
-(void) onResp:(BaseResp*)resp
{
    ...
    if([resp isKindOfClass:[SendMessageToWXResp class]])
    {
        NSString *strMsg = nil;
        if (resp.errCode == 0) {
            strMsg = NSLocalizedString(@"TKN_SHARE_WEIXIN_SUCCESS", @"");
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:nil message:@"XXXX" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alertView show];
            [alertView release];
        }else{
            strMsg = NSLocalizedString(@"TKN_SHARE_WEIXIN_FAILED", @"");
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (actionSheet.tag==1) {
        ...
        if (buttonIndex == 0) {
            ...
            [WXApi handleOpenURL:[NSURL URLWithString:@"wxf51c8154f251195f"] delegate:self];
